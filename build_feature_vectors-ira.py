...
class Options:
    ...
    def _init_parser(self):
        ...
        self.parser.add_argument(
            ...
            required=True,
            dest='tweets_file',
            help='The corpus of tweets (CSV) (may be gzipped)'
        )
        self.parser.add_argument(
            '-l', '--label',
            dest='label',
...
def build_f_vecs_for(hcc_info, hcc, accts, corpus_duration_d, f_vecs):
    ...
    for u_id in hcc_info['users']:
        ...
        # account features
        acct = accts[u_id]
        age_d = s_to_d(max(tweet_tss[u_id]) - utils.ts_2_epoch_seconds(acct['created_at'])) # s_to_ts(acct['created_at']))
...
def update_dummy_profile(acct, t):
    if acct == None:
        acct = {
            'created_at' : utils.parse_ts(t['account_creation_date'], utils.IRA_SHORT_TS_FORMAT),  # e.g. 2015-11-03
            'followers_count' : int(t['follower_count']),
            'friends_count':    int(t['following_count']),
            'statuses_count':   1
        }
    else:
        acct['followers_count'] = int(t['follower_count'])
        acct['friends_count']   = int(t['following_count'])
        acct['statuses_count'] += 1
    return acct
